...
void tiled_projection(const Tensor& x, const Weight& weight, Tensor& out, cudaStream_t stream) {
    ...
    using Schedule            = Q8SmallTMmaSchedule<Warps, TileColumns, Warps == 8 ? 2 : 3,
                                                    Q8SmallTMmaScaleAccess::Shared, Activation>;
    constexpr int SharedBytes = TileColumns > 64 ? sizeof(Q8SmallTMmaSharedStorage<Schedule>) : 0;
    if constexpr (SharedBytes > 0) {
        static const cudaError_t attribute = cudaFuncSetAttribute(
            q8_small_t_mma_kernel<Geometry, TileColumns, Schedule, Q8ContiguousOutput,
    ...
    q8_small_t_mma_kernel<Geometry, TileColumns, Schedule, Q8ContiguousOutput,
        ...
        <<<grid, Schedule::kThreads, SharedBytes, stream>>>(
            ...
            Q8SmallTMmaIdentityRows{}, columns);
