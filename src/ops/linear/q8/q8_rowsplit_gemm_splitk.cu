...
void launch_active_cols(const Tensor& x, const Weight& weight, Tensor& out, cudaStream_t stream) {
    ...
    using Schedule = Q8SmallTMmaSchedule<KWarps, TileCols, MinBlocks, ScaleAccess, ActivationCache>;
    static_assert((kRows % kRowsPerCta) == 0);
    const Q8ContiguousOutput output{static_cast<__nv_bfloat16*>(out.data), kRows};
...
void require_problem(const Tensor& x, const Weight& w, const Tensor& out) {
    if (x.ne[0] != kHidden || out.ne[0] != kRows || out.ne[1] != x.ne[1] || w.n != kRows ||
        ...
        throw std::invalid_argument("Q8 exact-T split-K requires [2048,16384]");
    }
}
...
void launch_q8_exact_t_splitk(const Tensor& x, const Weight& w, Tensor& out, cudaStream_t stream) {
    require_problem(x, w, out);
    if (x.ne[1] < kFirstExactCols || x.ne[1] > kLastExactCols) {
        throw std::invalid_argument("Q8 exact-T split-K requires T=2..48");
    }
    kLaunchers[x.ne[1] - kFirstExactCols](x, w, out, stream);
    CUDA_CHECK(cudaGetLastError());
}

void launch_q8_exact_t_composite(const Tensor& x, const Weight& w, Tensor& out,
                                 cudaStream_t stream) {
    require_problem(x, w, out);
    if (x.ne[1] < 33 || x.ne[1] > 127) {
        throw std::invalid_argument("Q8 exact-T composite requires T=33..127");
    }
    ...
    while (x.ne[1] - offset >= 32) {
        ...
        launch_q8_exact_t_splitk(x_slice, w, out_slice, stream);
        offset += 32;
    }
    ...
    if (tail == 1) {
        ...
        q8_rowsplit_decode_r16_launch(x_slice, w, out_slice, stream);
    } else if (tail >= 2) {
...
void launch_medium_route(const Tensor& x, const Weight& w, Tensor& out, cudaStream_t stream) {
    require_problem(x, w, out);
    if (x.ne[1] > TileCols) {
        throw std::invalid_argument("Q8 medium-T split-K route does not cover this T");
    }
    launch_medium<TileCols, KSplits, NGroups, MinBlocks>(x, w, out, stream);
    CUDA_CHECK(cudaGetLastError());
}

void launch_q8_dflash_medium(const Tensor& x, const Weight& w, Tensor& out, cudaStream_t stream) {
    require_problem(x, w, out);
    const int t = x.ne[1];
    if (t < 49 || t > 128) {
        throw std::invalid_argument("Q8 DFlash medium route requires T=49..128");
    }
