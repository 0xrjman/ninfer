...
__launch_bounds__(Schedule::kThreads, Schedule::kMinBlocksPerSm) void q8_grouped_ksplit_topk_kernel(
    ...
    union SharedStorage {
        struct {
            ...
            std::uint8_t scales[kRows][Schedule::kScaleAccess == Q8SmallTMmaScaleAccess::Shared
        ...
        const auto stage_activation = [&](int group_k0) {
            ...
            for (int item = lane; item < kItems; item += 32) {
                ...
                cp_async_zfill<16, Schedule::kActivationCache>(
                    &x_shared[warp][column * kTileK + q8_small_t_swizzle_64(column, k8 * 8)],
...
#pragma unroll
                for (int k_pair = 0; k_pair < 2; ++k_pair) {
                    ...
                    const unsigned a0 = q8_small_t_bf16_pair_from_s8(load_pair(gid, code_col));
                    const unsigned a1 = q8_small_t_bf16_pair_from_s8(load_pair(gid + 8, code_col));
                    const unsigned a2 = q8_small_t_bf16_pair_from_s8(load_pair(gid, code_col + 8));
                    const unsigned a3 =
                        q8_small_t_bf16_pair_from_s8(load_pair(gid + 8, code_col + 8));
#pragma unroll
                    for (int token_mma = 0; token_mma < kTokenMmas; ++token_mma) {
                        ...
                        ldmatrix_x2(
                            b0, b1,
                            smem_addr(
                                &x_shared[k_split][row * kTileK + q8_small_t_swizzle_64(
...
void launch_tile(const Tensor& hidden, const Weight& head, std::int32_t valid_rows,
    ...
    using Schedule = Q8SmallTMmaSchedule<8, Capacity, 2, Q8SmallTMmaScaleAccess::Shared>;
